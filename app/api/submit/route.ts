...
export async function POST(req: NextRequest) {
  ...
  // Prefer the record that actually has a Slack Channel ID (guard against duplicate project entries)
  const record = atData.records?.find((r: { fields: Record<string, unknown> }) => r.fields['Slack Channel ID']) ?? atData.records?.[0];
  ...
  if (!slackChannelId) return NextResponse.json({ error: 'No Slack channel found for this project — check that the project record in Airtable has a Slack Channel ID set' }, { status: 404 });
